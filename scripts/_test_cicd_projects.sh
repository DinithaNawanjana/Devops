#!/usr/bin/env bash
# Verify every CI/CD project's validate.sh passes against its reference
# solution. Flow: setup -> solution_cmds -> validate, /root remapped.
set -u
BASE="$(cd "$(dirname "$0")/.." && pwd)/content/projects/cicd"
overall=0
for P in "$BASE"/*/; do
  dir="$(basename "$P")"
  T="$(mktemp -d)"
